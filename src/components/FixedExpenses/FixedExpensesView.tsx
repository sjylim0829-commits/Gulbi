      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl bg-white p-5 border border-slate-200/80 shadow-xs space-y-2">
          ...
          <p className="text-xs text-slate-600 leading-relaxed font-medium">
            결제일 당일 [장부 기록] 버튼을 누르면 당월 지출 장부에 자동으로 일괄 이체 기록이 생성됩니다.
          </p>
        </div>
      </div>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        <button
                          onClick={() => {
                            if (confirm(`'${item.name}' 고정지출 항목을 삭제하시겠습니까?`)) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 border border-slate-200 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-slate-900">
              {editingItem ? '고정지출 항목 수정' : '새 고정지출 항목 추가'}
            </h3>

            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">고정지출 항목명</label>
                <input
                  type="text"
                  required
                  placeholder="예: 아파트 관리비, 월세, SKT 통신비, 넷플릭스"
                  value={name}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">월 금액 (원)</label>
                  <input
                    type="number"
                    required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl bg-slate-50 border border-slate-200 px-3 py-2.5 text-xs text-slate-900 focus:border-indigo-500 focus:bg-white focus:outline-none"
                  >
