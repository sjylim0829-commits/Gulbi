  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="rounded-3xl bg-slate-900 p-6 border border-slate-800 shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">매월 고정지출 관리</h1>
              <p className="text-xs text-slate-400">월세, 아파트 관리비, 공과금, 통신비, 보험료, 구독료 등 정기 지출 관리</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-1.5 rounded-xl bg-slate-950 px-3 py-1.5 border border-slate-800 text-xs">
              ...
              >
                <option value="day_asc" className="bg-slate-900 text-white">결제일 빠른순 🔺</option>
                <option value="day_desc" className="bg-slate-900 text-white">결제일 늦은순 🔻</option>
                ...
                <option value="name_asc" className="bg-slate-900 text-white">항목명 오름차순 (가-나-다)</option>
              </select>
            </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-2xl bg-slate-900 p-5 border border-slate-800 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400">매월 총 고정지출</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-rose-950/80 text-rose-400 border border-rose-800/60">
              <DollarSign className="h-4 w-4" />
            </div>
          </div>
          <div className="text-2xl font-extrabold text-rose-400 tracking-tight">
            {totalFixedExpenseAmount.toLocaleString()} <span className="text-sm font-normal text-slate-400">원 / 월</span>
          </div>
        </div>

        <div className="rounded-2xl bg-slate-900 p-5 border border-slate-800 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400">등록된 고정지출 항목</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-indigo-950/80 text-indigo-400 border border-indigo-800/60">
              <Wallet className="h-4 w-4" />
            </div>
          </div>
          <div className="text-2xl font-extrabold text-white tracking-tight">
            {fixedExpenses.length} <span className="text-sm font-normal text-slate-400">개 항목</span>
          </div>
        </div>

        <div className="rounded-2xl bg-slate-900 p-5 border border-slate-800 shadow-xs space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-400">스마트 관리 팁</span>
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-emerald-950/80 text-emerald-400 border border-emerald-800/60">
      ...
      <div className="rounded-3xl bg-slate-900 p-6 border border-slate-800/90 shadow-xs">
        {sortedFixedExpenses.length === 0 ? (
          <div className="py-12 text-center text-slate-500 text-sm space-y-2">
            <Clock className="mx-auto h-10 w-10 text-slate-600 mb-1" />
            <p className="font-semibold text-slate-300">등록된 고정지출 내역이 없습니다.</p>
            <p className="text-xs text-slate-500">월세, 관리비, 통신비, 보험료 등 매월 정기적으로 나가는 지출을 등록해 보세요!</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-800/80">
                {sortedFixedExpenses.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3.5 font-bold text-indigo-400">
                      <span className="rounded-lg bg-indigo-950/80 px-2.5 py-1 text-xs border border-indigo-800/60">
                    ...
                    <td className="px-4 py-3.5 text-right font-extrabold text-rose-400 text-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-3xl bg-slate-900 p-6 border border-slate-800 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-300 mb-1">고정 지출 항목명</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">월 결제 금액 (원)</label>
                  <input
                    type="number"
                    required
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">매월 결제 희망일 (1~31일)</label>
                  <input
                    type="number"
                    min="1"
                    max="31"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1">결제/이체 수단</label>
                  <input
                    type="text"
